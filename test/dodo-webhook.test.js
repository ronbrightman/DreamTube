...
test('payment.succeeded fires a server-side Purchase to BOTH PostHog and Meta CAPI, sharing the event_id from metadata.dreamtube_event_id, with value/currency/timestamp', async function () {
  ...
  assert.equal(phBody.properties.value, 1.99);
  ...
  assert.equal(metaEvent.custom_data.value, 1.99);
  assert.equal(metaEvent.custom_data.currency, 'USD');
  assert.ok(metaEvent.event_time, 'event_time should be present');
});

test('pack500 resolves price 8.95 for the Purchase event', async function () {
  await seedZeroBalance('purchase500@example.com');
  var spies = installAnalyticsFetchSpy();
  await handler(signedEvent(paymentPayload({
    payment_id: 'pay_analytics_500',
    product_cart: [{ product_id: 'pdt_pack500_test', quantity: 1 }],
    customer: { customer_id: 'cus_500a', email: 'purchase500@example.com' },
    metadata: { dreamtube_event_id: 'evt-500' }
  })));
  assert.equal(spies.posthogCalls[0].body.properties.value, 8.95);
  assert.equal(spies.metaCalls[0].body.data[0].custom_data.value, 8.95);
});
