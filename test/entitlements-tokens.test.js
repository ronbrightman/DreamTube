...
// core: getTokenStatus (the 170-token first-ever-read grant, and the
...
// late July, then -> 170 on 2026-08-09 (founder-directed economy retune). 170
// funds ONE onboarding video (100 tokens) and leaves 70 -- short of a second;
// reaching a second video requires the engagement bonuses (first-claim +20 ->
// 90, then email-verify OR add-to-homescreen +20 -> 110), which is the point:
// the token wall drives those valuable actions. Unaffected by the
...
test('a never-before-seen email gets the 170-token signup grant on first read, and it is actually persisted', async function () {
  var ev = fakeEvent({ ip: nextIp() });
  var status = await entitlements.getTokenStatus(ev, 'brandnew@example.com');
  assert.equal(status.balance, 170);
...
test('reading again right away does not re-grant — balance stays at 170', async function () {
  var ev = fakeEvent({ ip: nextIp() });
...
test('getTokenStatus never itself grants the daily +20 -- reading repeatedly leaves the balance untouched even though claimable is true', async function () {
  var ev = fakeEvent({ ip: nextIp() });
  await entitlements.getTokenStatus(ev, 'readonly@example.com'); // -> 170, claimable
  ...
  assert.equal(status.balance, 170);
  assert.equal(status2.balance, 170, 'reading is not claiming -- only claimDailyTokens grants');
});

test('claimable is false, and nextClaimAt is in the future, right after a claim', async function () {
  var ev = fakeEvent({ ip: nextIp() });
  await entitlements.getTokenStatus(ev, 'justclaimed@example.com'); // -> 170
...
test('getTokenStatus for a not-yet-claimed account returns dailyClaimAmount: 20 (first claim equals the normal amount after the 2026-08-08 retune)', async function () {
  var ev = fakeEvent({ ip: nextIp() });
  var status = await entitlements.getTokenStatus(ev, 'neverclaimedyet@example.com'); // -> 170, never claimed
  assert.equal(status.dailyClaimAmount, 20);
});

test('getTokenStatus for an already-claimed-once account returns dailyClaimAmount: 20 (the normal amount)', async function () {
  var ev = fakeEvent({ ip: nextIp() });
  await entitlements.getTokenStatus(ev, 'claimedonce@example.com'); // -> 170
...
test('spendTokens deducts the given amount from balance', async function () {
  var ev = fakeEvent({ ip: nextIp() });
  await entitlements.getTokenStatus(ev, 'spender@example.com'); // -> 170
...
test('spendTokens preserves lastClaimAt/streak untouched -- a spend must never reset the claim cooldown or streak', async function () {
  var ev = fakeEvent({ ip: nextIp() });
  await entitlements.getTokenStatus(ev, 'spendclaim@example.com'); // -> 170
  ...
  assert.equal(record.tokens.balance, 90, '170 + 20 first-ever-claim bonus - 100 spent');
...
test('the Nth+1 brand-new email from the same IP in one day gets balance 0 instead of the 170 signup grant', async function () {
  ...
  assert.equal(status1.balance, 170);
  assert.equal(status2.balance, 170);
...
test('each IP gets its own daily cap bucket — a different IP is unaffected by another IP already being over the limit', async function () {
  ...
  assert.equal(freshOnB.balance, 170);
});
...
test('the per-IP cap only ever applies to the first-ever read for an email — an already-initialized account is never re-checked against it', async function () {
  ...
  assert.equal(status.balance, 170, 'unaffected by the IP being over its new-signup cap');
});
...
test('addTokens credits the given amount onto the existing balance', async function () {
  var ev = fakeEvent({ ip: nextIp() });
  await entitlements.getTokenStatus(ev, 'topup@example.com'); // -> 170
...
test('addTokens on a never-before-seen email materializes the record starting from the usual signup grant, then adds on top', async function () {
  ...
  assert.equal(record.tokens.balance, 470, '170 signup grant + 300 top-up');
});
...
test('addTokens does not change lastClaimAt/streak (purely additive to balance, never resets or delays the next daily claim)', async function () {
  var ev = fakeEvent({ ip: nextIp() });
  await entitlements.getTokenStatus(ev, 'timing-topup@example.com'); // -> 170
  ...
  assert.equal(record.tokens.balance, 290, '170 + 20 first-ever-claim bonus + 100 top-up');
...
// init cap and then clobbered its own 170 grant with a capped 0.
...
test('a replayed init (grant marker present, entitlement write not yet visible) echoes the grant and burns no IP slot', async function () {
  ...
  mockBlobs.seed('dreamtube-rate-limits', 'token-init-grant:replayed@example.com', { balance: 170, at: 1 });
...
test('a capped init persists NOTHING - the same email gets its full grant once the cap clears', async function () {
  ...
  assert.equal(recovered.balance, 170);
});
...
test('a straggling reader whose init marker HAS propagated but whose OWN entitlement read is still stale must not echo the marker\'s frozen balance once a claim already landed on top of it', async function () {
  ...
  // Real, already-committed state: the 170-token init grant landed, and a
  // claim's own separate credit step already bumped it to 190 -- e.g. the
  // exact incident this branch targets, where a claim's own internal
  // syncTokens call writes the init marker (170, frozen) synchronously
  // BEFORE the claim's separate credit step bumps the real balance to 190.
  await entitlements.setEntitlement(ev, email, {
    tokens: { balance: 190, lastClaimAt: Date.now(), streak: 1 }
  });
  mockBlobs.seed('dreamtube-rate-limits', 'token-init-grant:' + email, { balance: 170, at: Date.now() });
  ...
  // fix's own re-derive read) sees the real, already-committed 190:
  ...
  assert.equal(status.balance, 190, 'must re-derive the real, already-credited balance from a fresh read -- not echo the marker\'s frozen 170 and understate a claim that already landed on top of it');
});
...
test('the marker-echo path still falls back to the marker\'s frozen value when a re-derive read ALSO still shows no tokens (propagation still lagging both reads)', async function () {
  ...
  assert.equal(status.balance, 170, 'falls back to the marker\'s frozen value when a fresh re-derive read still shows no tokens either -- the narrowing reduces the stale window, it does not require the real value to always be available');
});
