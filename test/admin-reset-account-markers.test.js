...
// that a re-signup after a reset genuinely re-grants the 220 base + re-earns
...
/** Materializes a full set of once-ever markers for `email`: the 220-token base grant (writes the token-init-grant marker + record), plus the install and verify-email achievement grants (each writes its outer marker + bumps balance). */
async function seedFullAccountState(event, email) {
  await entitlements.getTokenStatus(event, email); // -> 220, writes token-init-grant marker
...
test('after a reset, a re-signup genuinely re-grants the 220 base and re-earns the install bonus', function () {
  return withEnv({ OWNER_EMAIL: OWNER_EMAIL }, async function () {
    var ev = fakeEvent({ method: 'POST' });
    await seedOwner(ev);
    var target = 'testacct@example.com';
    ...
    assert.equal(status.balance, 220, 'a fresh read after the reset re-grants the full 220 base, not an echoed/stale value');
    ...
    assert.equal(regrant.balance, 240, '220 re-granted base + 20 re-earned install bonus');
  });
});
...
test('a NORMAL entitlement deletion (delete-account.js teardown) LEAVES the token-init-grant marker — so a re-signup cannot re-farm the base grant', function () {
  return withEnv({ OWNER_EMAIL: OWNER_EMAIL }, async function () {
    ...
    // A re-read therefore does NOT re-grant a fresh 220 record: the surviving
