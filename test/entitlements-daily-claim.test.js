...
test('claimDailyTokens on a brand-new email claims immediately: +20 balance, streak 1, nextClaimAt ~20h out', async function () {
  ...
  await entitlements.getTokenStatus(ev, 'brandnewclaim@example.com'); // materializes the 150-token record
  var before = Date.now();
  var result = await entitlements.claimDailyTokens(ev, 'brandnewclaim@example.com');
  assert.equal(result.claimed, true);
  assert.equal(result.balance, 170, '150 + 20 (2026-08-08 retune: the first-claim bonus was retired to equal the normal daily 20)');
...
test('claimDailyTokens works even when getTokenStatus/syncTokens was never called first -- a genuinely fresh email can claim directly, still getting the first-claim bonus', async function () {
  ...
  assert.equal(result.claimed, true);
  assert.equal(result.balance, 170, '150 signup grant (materialized lazily inside claimDailyTokens itself) + 20 first-ever-claim grant');
...
test('claiming twice in a row -- the second claim is rejected as not-yet-claimable, with the SAME shape the spec requires', async function () {
  ...
  assert.equal(record.tokens.balance, 170, 'balance must NOT have moved on the rejected second attempt (150 + 20 from the first, successful claim)');
});
...
test('a brand-new account\'s very first claim grants exactly 20 and stamps a top-level firstClaimAt', async function () {
  ...
  await entitlements.getTokenStatus(ev, email); // -> 150, no lastClaimAt/firstClaimAt yet

  var before = Date.now();
  var result = await entitlements.claimDailyTokens(ev, email);
  assert.equal(result.claimed, true);
  assert.equal(result.amountClaimed, 20);
  assert.equal(result.balance, 170);
...
test('that same account\'s SECOND claim (after the cooldown) also grants 20, and firstClaimAt is never overwritten', async function () {
  ...
  await entitlements.getTokenStatus(ev, email); // -> 150
  ...
  assert.equal(second.balance, 170 + 20);
...
test('claiming daily just under 48h apart, three days running, keeps incrementing the streak', async function () {
  var ev = fakeEvent({ ip: nextIp() });
  await entitlements.getTokenStatus(ev, 'multiday@example.com'); // -> 150

  var day1 = await entitlements.claimDailyTokens(ev, 'multiday@example.com');
  assert.equal(day1.streak, 1);
  assert.equal(day1.balance, 170, '150 + 20 first-ever-claim grant');
  ...
  assert.equal(day3.balance, 150 + 20 * 3, 'each of day1/day2/day3 grants the normal 20 (first-claim bonus retired 2026-08-08)');
});
...
test("a claim whose OWN first CAS read does not yet see the real, already-seeded signup grant (a stale read landing just before propagation) is atomically REJECTED and retries against the real balance -- must not silently drop the signup grant", async function () {
  ...
  // by the time that loop runs the real 150-token grant already exists
  // server-side.
  ...
  try {
    ...
    assert.equal(result.balance, 170, '150 (real signup grant, already committed before this loop started) + 20 (first-ever-claim grant) -- must NOT silently drop the signup grant just because attempt 1\'s own read raced the real, already-committed state');
  } finally {
    mockBlobs.clearCasReadOverride(entitlements.STORE_NAME);
  }

  var record = await entitlements.getEntitlement(ev, email);
  assert.equal(record.tokens.balance, 170, 'the persisted record must reflect the real seeded balance too, not just this call\'s return value');
});
...
// stayed at the pre-claim 150 instead of 170, and only "healed" minutes
...
test('a straggling duplicate signup-init write must not clobber a claim that already landed (guarded init write, not a blind full-object replace)', async function () {
  ...
  assert.equal(claimResult.balance, 170);
  ...
  assert.equal(status.balance, 170, 'must echo the REAL current balance, never fabricate/revert to a fresh 150');

  var record = await entitlements.getEntitlement(ev, email);
  assert.equal(record.tokens.balance, 170, 'the persisted record must still reflect the claim');
...
// above fails with `150 !== 170` (the straggler's blind `tokens: fresh`
...
// entry points (a plain balance read that lazily seeds the 150-token
...
test('a genuinely concurrent signup-seed read (getTokenStatus) and first-ever claim (claimDailyTokens) for the SAME brand-new email both land correctly -- no lost update either way', async function () {
  ...
  var results = await Promise.all([
    entitlements.getTokenStatus(ev, email),   // lazily seeds the 150-token signup grant
  ...
  // BOTH the signup grant and the claim -- 170 total, never a lower number
  ...
  var record = await entitlements.getEntitlement(ev, email);
  assert.equal(record.tokens.balance, 170, '150 (signup grant) + 20 (first-ever-claim grant) must BOTH have landed -- neither write may silently erase the other');
...
test('repeated trials of the signup-seed-vs-claim race stay consistent (rules out a lucky single pass)', async function () {
  for (var i = 0; i < 8; i++) {
    ...
    var record = await entitlements.getEntitlement(ev, email);
    assert.equal(record.tokens.balance, 170, 'trial ' + i + ': both the signup grant and the claim must land');
  }
});
