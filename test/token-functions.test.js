...
// materialization point for a brand-new email's 320-token signup grant).
...
test('GET with a brand-new email materializes the 320-token signup grant, claimable immediately', async function () {
  var res = await getTokenStatusHandler(fakeEvent({ method: 'GET', ip: nextIp(), query: { email: 'fresh@example.com' } }));
  assert.equal(res.statusCode, 200);
  var body = JSON.parse(res.body);
  assert.equal(body.balance, 320);
...
test('the real request IP is what the per-IP new-grant cap keys off, not a shared default', async function () {
  ...
  assert.equal(JSON.parse(res1.body).balance, 320);
