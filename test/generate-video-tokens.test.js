...
// brand-new email's 170-token signup grant materializes correctly through
...
test('a brand-new email (never seen before) gets its 170-token signup grant materialized right here and proceeds', async function () {
  stubFetchOk();
  ...
  assert.equal(record.tokens.balance, 70, '170 granted, 100 spent on this successful generation');
});
