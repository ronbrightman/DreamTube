...
// brand-new email's 150-token signup grant materializes correctly through
...
test('a brand-new email (never seen before) gets its 150-token signup grant materialized right here and proceeds', async function () {
  stubFetchOk();
  ...
  assert.equal(record.tokens.balance, 50, '150 granted, 100 spent on this successful generation');
});
