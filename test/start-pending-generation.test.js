...
test('a brand-new email gets the 150-token signup grant and a successful submission spends 100 of it, returning pendingId + operationName', async function () {
  stubFetchOk();
  ...
  assert.equal(status.balance, 50); // 150 granted - 100 spent
...
test('mediaType "image": a brand-new email gets the 150-token signup grant and a successful submission spends only 10 of it', async function () {
  stubFetchOk();
  ...
  assert.equal(status.balance, 140); // 150 granted - 10 spent
