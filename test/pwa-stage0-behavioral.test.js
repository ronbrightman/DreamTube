...
test('apple-touch-icon is present and fetchable on every real app page (iOS uses this specifically, not just manifest icons)', async function (t) {
  ...
  try {
    ...
    for (var i = 0; i < pagesToCheck.length; i++) {
      ...
      assert.equal(href, 'assets/apple-touch-icon-v4.png', pagesToCheck[i] + ' must declare a real apple-touch-icon');
      ...
      var fetchStatus = await page.evaluate(function () {
        return fetch('assets/apple-touch-icon-v4.png').then(function (r) { return r.status; });
      });
      assert.equal(fetchStatus, 200, 'assets/apple-touch-icon-v4.png must actually be fetchable, not a dangling link');
    }
  } finally {
    await context.close();
  }
