...
test('verify-password-reset: two concurrent resets for the SAME account (e.g. two valid links open on two devices) -- both apply (no conflict detection, see the account store\'s own header comment), and the account is usable with whichever password actually landed last', async function () {
  var registerHandler = require('../netlify/functions/register-account').handler;
  ...
  // The account must actually be usable with whichever password landed
  // last -- never left in a broken/unusable state.
  var accountStore = require('../netlify/functions/lib/account-store');
  var event = fakeEvent({ method: 'POST' });
  var stored = await accountStore.getByUsername(event, 'tara');
  var login = await accountStore.verifyLogin(event, 'tara', stored.password);
