...
// existing item — id/category/title/detail are seed-authored content, not
// something the UI lets anyone mutate (see that function's own doc comment
// for why). `comment` is the one owner-writable field that isn't a status
// flag: a free-text note the founder leaves for whoever maintains this
// tracker's content (title/detail wording) to read and act on later — e.g.
// "actually this is lower priority" or extra context — then clear once
// addressed. It's blank ("") for every seed item; nothing seeds a comment.
