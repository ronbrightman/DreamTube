...
// tracker.html's Reviewed button). Deliberately modeled exactly like
// `startedAt`: a one-way signal, no "un-review" control anywhere, same
// reasoning as `started` (see update-tracker-item.js's own doc comment)
// — this is a founder review checkpoint, not a toggle. It's independent
// of `done`/`doneAt`: an item can be un-done and re-done without ever
// clearing a `reviewedAt` that was already set, and tracker.html's own
// "done since last review" digest counts against `reviewedAt`, not
// against `done` alone (see tracker.html's reviewCutoff logic).
//
...
//
// `createdAt`/`doneAt`/`startedAt`/`reviewedAt` are all `null` for every
// seed item — there's no real historical timestamp to backfill for
// content that predates this field existing at all, and a fabricated one
// would be actively misleading (see tracker.html for how a `null`
// renders: the timestamp line is simply omitted rather than showing a
// fake date). New items created through add-tracker-item.js do get a
// real `createdAt` going forward (see addItem below); `doneAt`/
// `startedAt`/`reviewedAt` are set the moment an item actually
// transitions to done/started/reviewed, for items both old and new (see
// updateItem below).
//
...
//   - missing createdAt/doneAt/startedAt/reviewedAt all default to `null`
//     — no fabricated history for content that predates these fields.
//     `reviewedAt` was added in the same "backfill missing fields to
//     null" pass as the other three (see the Reviewed-marker build this
//     comment was extended for) — the live store was already seeded
...
function migrateItem(item) {
  ...
  ['createdAt', 'doneAt', 'startedAt', 'reviewedAt'].forEach(function (key) {
 ...
 *   reviewed: true — a one-way "founder has reviewed this" signal (see
 *     tracker.html's Reviewed button), same shape as `started`: sets
 *     `reviewedAt` to the current server time the first time this fires;
 *     on an item that already has a `reviewedAt`, this is a no-op — there
 *     is no "un-review" exposed by this endpoint either. Deliberately NOT
 *     gated on `done` server-side (tracker.html's own UI only ever shows
 *     the Reviewed button on a done item, but this function trusts its
 *     caller the same way it trusts every other patch field here).
...
async function updateItem(event, id, patch) {
  ...
  await writeItemsWithRetry(
    ...
    function (items) {
      ...
      if (Object.prototype.hasOwnProperty.call(patch, 'reviewed') && patch.reviewed && !current.reviewedAt) {
    ...
    function (items) {
      ...
      if (Object.prototype.hasOwnProperty.call(patch, 'reviewed') && patch.reviewed) {
        if (found.reviewedAt !== updated.reviewedAt) return false;
      }
      if (Object.prototype.hasOwnProperty.call(patch, 'newComment')) {
 ...
 * comments: [], doneAt: null, startedAt: null, reviewedAt: null. Unlike
 * SEED_ITEMS' fallback `createdAt: null` (no real history to record for
