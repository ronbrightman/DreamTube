...
var READING_SPEED = 0.9; // kokoro FALLBACK tier only, unaffected by the ElevenLabs switch (founder-adjusted 08-04, "slightly too slow" at 0.8 on his full walk). ElevenLabs' own pace is governed by that tier's own default settings — see the header comment's "Reading pace" note: the founder's approved ElevenLabs sample used DEFAULT speed/stability (no override sent here), a deliberate choice to match what he actually heard rather than a guessed multiplier. No real-audio access in this sandbox to verify the resulting pace against the same 0.9 "feel" kokoro was tuned to — flagged for whoever next has real listening access to confirm/adjust.
...
exports.handler = async function (event) {
  ...
  try {
    var elBody = JSON.stringify({ text: text, voice: persona.voiceId, timestamps: true });
