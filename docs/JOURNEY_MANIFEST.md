...
| 0.5 | Entry chooser (fresh entries only) | `wizard.html` — `showChooser` (a pre-step, deliberately NOT in `SCREEN_RENDERERS`) | "What did you dream?" — 🪄 Build it step by step / ✍️ Write it in your own words / 🎙️ Speak it (founder round 8 on the Layout-B redesign: "Get started skips the choosing between the 3 options"). Build → step 1; Write → jumps to step 6 (free text) and continues normally from there; Speak → `create.html?record=1` via its login gate (hidden entirely in FB/IG webviews, mirroring create.html's record guard). Shown only to fresh entries (`?entry=index` or direct) — never to Facebook return legs or `?resume=1` arrivals. Fires `wizard_entry_mode_chosen {mode, surface:'wizard'}` only — no `wizard_step_viewed`, so existing step numbering below is untouched. |
| 1 | Subject | `wizard.html` — `renderSubject` | "Who was in the dream?" (Layout-B copy) — multi-select; tapping Me selects it AND opens the character sheet in self mode (describe + photo upload, Cancel keeps Me selected with no details — founder round 8 + the 08-04 details-optional ruling); "Someone I know" rows keep their inline optional detail input. |
| 2 | Setting | `wizard.html` — `renderSetting` | "Where did it happen?" |
| 3 | Action | `wizard.html` — `renderAction` | "What was happening?" — required, no skip. |
| 4 | Mood | `wizard.html` — `renderMood` | "How did the dream feel?" |
| 5 | Style | `wizard.html` — `renderStyleStep` | Layout-B pill rows (replaced the embedded style-card grid on this wizard only), auto-advances on pick. |
| 6 | Free text | `wizard.html` — `renderFreeText` | Optional escape hatch — and Write mode's landing step (see 0.5). |
| 7 | Recap (editable) | `wizard.html` — `renderRecap` | "Here's your dream — make it yours" — the editable dream text (pencil affordance, "Edit anything before we bring it to life.", "Looks good →" CTA) on its OWN screen before the wall (founder round 8, restoring the approved mock's structure). Fires the sanctioned `wizard_recap_viewed {}` instead of a renumbered `wizard_step_viewed` — every pre-existing step keeps its historical step value (wall stays `step:7`), see `wizard.html`'s `STEP_ANALYTICS`. |
| 8 | Signup wall (hybrid, screen-13 parity) | `wizard.html` — `renderSignupWall` | ONE merged screen replacing the former Contact-capture + username/password Signup pair (founder order, tracker item `for-product-wizard-signup-wall-is-the-ol-lt1l9j`): the forming-veil "your dream is forming" indicator, the (flag-gated) Facebook Login button + "or" divider, and a single passwordless email field — the same hybrid wall `start.html`'s live screen 13 uses. Email deliverability-checked (`check-email.js`), then the real, billed pending-generation call fires in parallel with `DreamStore.signupPasswordless` — BEFORE any account exists; an already-registered email resolves via the same enter-the-code step as screen 13. Success claims + adopts the running job. |
...
**Extracted/asserted array:** `wizard.html`'s `SCREEN_RENDERERS = [renderSubject, renderSetting, renderAction, renderMood, renderStyleStep, renderFreeText, renderRecap, renderSignupWall]`.
