...
| Real fal.ai video cost *(at the time of this spec, 2026-07-24)* | Veo 3.1 Fast ≈ $0.10–0.20/sec × 8s ≈ **$0.80–$1.60/video** — **superseded 2026-07-28**: standard text-to-video now defaults to Veo 3.1 Lite, ≈ $0.24–$0.40/video (see `generate-video.js` header comment and tracker item for-product-switch-default-video-model-t-lqxafa); the comparisons below are historical, from when Fast was the only path | `generate-video.js` header comment |
...
8. **`wizard.html`** — *(STALE AS WRITTEN, 2026-08-05: the merged passwordless signup wall — tracker item `for-product-wizard-signup-wall-is-the-ol-lt1l9j` — replaced `renderContact`/`renderSignup` with one `renderSignupWall`; if this shelved spec is ever picked up, the equivalent edits land in `renderWallEmailStep`'s payload + `completeSignupAndAdvance`'s `adoptPendingGeneration` call instead.)* Step 7 (`renderContact`)'s fetch to `start-pending-generation.js` adds `mediaType: 'image'` (hardcoded — every wizard run is image-first now, this isn't a user choice at this point in the flow); the `DreamStore.setDraft(...)` call right after adds `mediaType: 'image'`; `renderSignup`'s `DreamStore.adoptPendingGeneration(pendingOperationName, pendingStartedAt, assembled.caption, chosenStyle)` call gets a 5th arg, `'image'`. Copy change at line 893 (§6).
...
- **Where:** `result.html`, directly below the caption/tag row, above the interpretation pill and the compact CTA/quiet-link rows below it (see the result.html first-video-screen redesign, tracker.html's `for-product-build-result-html-first-vide-mupiua`) — the single most prominent action on the screen, deliberately kept full-size/warm-styled as that redesign's one explicit exception to its general CTA-shrinking. Shown only when `dream.imageUrl && !dream.videoUrl`.
