...
| **Why this exists** | Built in the SAME release as Dreamer Pass go-live (tracker item `for-product-build-conversion-tracking-fo-k5ow3q`) specifically so no subscription revenue goes untracked from day one — the pack path already had durable server-side Purchase tracking; the subscription did not. The Pass itself is still owner-gated/diagnostic as of this instrumentation landing — this is the tracking wired ahead of that go-live, not a claim that the Pass is purchasable by real customers yet |
