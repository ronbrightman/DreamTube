...
- **Ron-facing output:** **bold EVERY line/point Ron actually needs to read** —
  ...
  read.")
- **CREATION FLOWS ARE NOW CONSOLIDATED — ONE FUNNEL for organic + ads; `create.html` for logged-in (founder-directed 2026-08-14, "unify all creation flows").** This SUPERSEDES the old "keep three surfaces identical" rule (which caused recurring fix-one-forget-the-others bugs — the chooser, the classifier drift). Current architecture:
  - **Organic + paid ads both go through the ONE growth funnel** (`dreamtube-growth` `index.html`, served same-origin at `dreamtube.life/go/` via the `netlify.toml` `/go/*` proxy). The app homepage `index.html` "Get Started" redirects to `/go/?utm_source=organic`; ads point at the funnel too. The funnel builds the dream and hands off (`?resume=1&caption=`) into the app's signup wall.
  - **`wizard.html` is RETIRED to a funnel-arrival receiver.** An anonymous bare/direct hit redirects to `/go/`; a signed-in bare hit → `create.html`; the ONLY live path through it is the funnel handoff → signup wall (with the pre-email content gate, identity stitch, Meta cookies, character-stash intact). Its old chip-build creation UI is retained-but-dead markup (kept only for `SCREEN_RENDERERS` positional-index / `wizard_step_viewed {step:7}` analytics stability). Do NOT resurrect it as a creation surface.
  - **Logged-in creation stays on `create.html`** (founder decision 08-14: keep it, don't force logged-in through the acquisition funnel). It already shares the classifier, so the real drift is gone. The bottom-nav "+", Home Tonight CTA, "Make another", chamber "Begin" etc. all still point at `create.html`.
  - **THE DRIFT-KILLER: the funnel now LOADS the app's shared `js/wizard-chips.js`** (a byte-identical vendored copy in the growth repo, `<script src="js/wizard-chips.js">`), instead of the old hand-copied inline `inferMoodFromText`/`MOOD_KEYWORDS`. `test/wizard-chips-sync.test.js` (growth repo) fails CI the moment the vendored copy and the app original diverge. So the classifier can no longer silently drift by construction — but if you edit the app's `js/wizard-chips.js`, you must re-vendor the byte copy into `dreamtube-growth/js/wizard-chips.js` in the same pass (the sync test enforces this). The funnel's caption/recap WORDING, camera and lighting tables stay deliberately funnel-specific (different phrasing) and are NOT shared.
  - **The canonical flow shape is still the SHORT 3-question Build (Subject → Action → free text; Setting inferred; Mood inferred from text via `WizardChips.inferMoodFromText`, defaults 'dreamy')** — a "Build it / Write it / Record it" mode chooser precedes it in both the funnel and `create.html`. Mood drives BOTH the music bed (js/store.js startGeneration backstop) AND the video prompt (`assembleCaption` via the opt-in `moodInferText`; the human recap story keeps its default mood word). Do NOT re-add Setting/Mood steps without founder instruction.
...
- **When you FIND a problem, FIX it — don't ask permission first (founder standing rule 08-14: "always fix without asking me").** Surfacing a bug and asking "want me to fix it?" is the wrong move — root-cause and fix it in the same pass, then report what you found AND that it's fixed. This does NOT override the real approval gates (spending money, launching/pausing campaigns, vendor picks, auth-sensitive or irreversible/outward-facing actions) — those still need explicit sign-off. But for ordinary bugs, tracking gaps, dead features, and broken metrics: just fix them.
